...
Run once from this repo (requires Node 20+):
...
# Install and use Node 18 (required for this project's webpack version)
nvm install 18
nvm use 18
...
To clean up later: `nvm uninstall 18` removes the Node 18 install entirely.
...
```
REACT_APP_BROKER_URL=https://dnd-battle-map.<your-subdomain>.workers.dev
```
...
```bash
nvm use 18
REACT_APP_BROKER_URL=https://dnd-battle-map.<your-subdomain>.workers.dev npm start
```
...
| Stuck on "joining…" | `REACT_APP_BROKER_URL` not set, or worker not deployed yet |
...
| Build fails | Make sure you're on Node 18 (`nvm use 18`) |
