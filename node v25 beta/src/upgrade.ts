...
import blobToBuffer, { toBlobPart } from "./helpers/blobToBuffer";
...
export const versions: Record<number, VersionCallback> = {
  ...
  2(v, onUpgrade) {
    v.stores({}).upgrade(async (tx) => {
      ...
      let mapBuffers: Record<string, Uint8Array> = {};
      for (let map of maps) {
  ...
  15(v, onUpgrade) {
    v.stores({}).upgrade(async (tx) => {
      ...
      for (let token of tokens) {
        const url = URL.createObjectURL(new Blob([toBlobPart(token.file)]));
        let image = new Image();
...
async function createDataThumbnail(data: any): Promise<{
  ...
  if (data?.resolutions?.low?.file) {
    url = URL.createObjectURL(
      new Blob([toBlobPart(data.resolutions.low.file)])
    );
  } else {
    url = URL.createObjectURL(new Blob([toBlobPart(data.file)]));
  }
...
async function createDataOutline(
  ...
  const url = URL.createObjectURL(new Blob([toBlobPart(data.file)]));
  return await Dexie.waitFor(
    new Promise((resolve) => {
      let image = new Image();
      image.onload = async () => {
